...
if($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["submit"] === "apply"){
	...
	if(!empty($_FILES["cv"]["name"])){
		...
		if($cvFileType != "docx" && $cvFileType != "pdf") {
			$inserts_error["cv_err"] = "Sorry, only DOCX & PDF files are allowed.";
		} 
		else {
	...
	if(empty($inserts_error)){
		$sql_request = "INSERT INTO applications(user_id, job_id, custom_message, cv) 
						VALUES ('".$user['id']."',  '". $job['id']."',  '". $insert_user['custom_message']."' ,'". $insert_user['cv']."')";
					...
					<form name="apply" action="" method="POST" enctype="multipart/form-data">
						<div class="flex-container justified-horizontally flex-wrap">									
							...
							<div class="form-field-wrapper width-large">
								...
								<?php if(!empty($inserts_error["cv_err"]))
									echo $inserts_error["cv_err"];
								?>
							</div>
						</div>	
